...
#### 1.1.1 `strings` for searching, replacing, comparing, trimming, splitting and joining strigns
...
#### 1.1.2 `bytes` for searching, replacing, comparing, trimming, splitting and joining `[]byte`
...
#### 1.1.3 `strconv` for converting bool, integer, floating-point values to and from string
...
#### 1.1.4 `unicode` for classifying rune
...
### 1.3 Map
`counts := make(map[string]int)`, the map type is a **reference** to the underlying data structure.
