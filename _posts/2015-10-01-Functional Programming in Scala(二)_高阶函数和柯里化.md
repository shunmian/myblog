...
>**柯里化**：将**n参数函数**转成**单参数函数**的过程。方法是通过不断转换成一个包含**n个单参数的函数**的嵌套**树形结构**。每一个树形结构的**上层节点**的输入参数对所有**下层节点**的函数**可见**。
...
最后将Currying总结成一句话：在单参数的前提下，函数式编程可以实现所有计算(包括多参数函数，方法是通过Currying，将`f(arg1,arg2,...,argn)`转成`f(arg1)(arg2)...(argn)`的形式，使得我们在应用该函数的时候，满足单参数的前提)。
