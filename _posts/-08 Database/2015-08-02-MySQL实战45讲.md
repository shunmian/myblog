...
{: .img_middle_hg}
![regular expression](/assets/images/posts/-08 Database/MySQL实战45讲/索引下推优化.png)
...
> A: PRIMARY KEY (`a`,`b`), KEY `c` (`c`), KEY `ca` (`c`,`b`)必须；KEY `ca` (`c`,`a`)不必须因为最左前缀原则PRIMARY KEY (`a`,`b`),已经是按a排序了。
