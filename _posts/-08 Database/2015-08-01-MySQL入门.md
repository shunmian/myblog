...
- `type`: how sql will access the rows in the table, Worse to better: `ALL` < `index` < `range`(where film_id >= 10 and film_id <= 20) < `ref` < `eq_ref` < `const`(where film_id = 1) < `system` < `NULL`(select max).
